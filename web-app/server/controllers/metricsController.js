...
metricsController.getFEData = (req, res, next) => {
  ...
  try {
    db.query(metricsQuery, value)
      .then(data => {
        ...
        if (filteredData.length > 0) {
            res.locals.performance = filteredData[filteredData.length - 1].performance;
        }
        return next();
      });
  } catch (err) {
    return next({
      log: 'metricsController.getData - error getting FE data',
      status: 500,
      message: { err: 'metricsController.getData - error getting FE data' + err.message }
    });
  }
};
...
metricsController.getBEData = (req, res, next) => {
  ...
  try {
    db.query(metricsQuery, value).then(data => {
      ...
      res.locals.BEmetrics = filteredData;
      if (filteredData.length > 0) {
          res.locals.response = filteredData[filteredData.length - 1].average_response_time;
      }
      return next();
    });
  } catch (err) {
    return next({
      ...
      status: 500,
      message: { err: 'metricsController.getData - error getting BE data' + err.message },
    });
  }
};
...
metricsController.postData = (req, res, next) => {
  try {
    ...
    return next({
      log: 'metricsController.postData - error adding project data: ' + err,
      status: 500,
      message: { err: 'metricsController.postData - error adding project data:' + err },
    });
  }
};
